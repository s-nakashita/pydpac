...
while itrial < ntrial:
    itrial += 1
    logger.info(f"== trial {itrial} ==")
    savefig=False
    if itrial <= 10:
        savefig=True
        figdir = figdir_parent / f'test{itrial}'
    ...
    ## observations
    yobs = obsope.add_noise(obsope.h_operator(ix_lam[1:-1], u0_t))

    ## analysis
    _ = var_nest.calc_pf(u0_lam[1:-1],B_lam,0)
    ua_lam = up_lam.copy()
    ua_lam_nest = up_lam.copy()
    ua_gm, _, _, _, _, _ = var_gm(u0_gm, B_gm, yobs, ix_lam[1:-1])
    ua_lam[1:-1], _, _, _, _, _ = var_lam(up_lam[1:-1], B_lam, yobs, ix_lam[1:-1])
    ua_lam_nest[1:-1], _, _, _, _, _ = var_nest(up_lam[1:-1], B_lam, yobs, ix_lam[1:-1], u0_gm)
    ...
    ya_lam_nest = dst(ua_lam_nest[1:-1],type=1)/nx_lam
    ...
    yb_lam = dst(errb_lam[1:-1],type=1)/nx_lam
    ya_lam = dst(erra_lam[1:-1],type=1)/nx_lam
    ya_lam_nest = dst(erra_lam_nest[1:-1],type=1)/nx_lam
...
ax.set_xlabel('trial')
ax.set_ylabel('RMSE')
ax.set_title(f'ntrial={ntrial}, 3DVar')
fig.savefig(figdir_parent/'rmse.png',dpi=300)
plt.show()
...
axs[1].legend()
axs[1].set_xlabel('wave number k')
fig.suptitle(f'ntrial={ntrial}, 3DVar')
fig.savefig(figdir_parent/'errspec.png',dpi=300)
plt.show()