...
$$
\langle \boldsymbol{\varepsilon}^\mathrm{b} \boldsymbol{\varepsilon}^\mathrm{vT} \rangle = a\langle\boldsymbol{\varepsilon}^\mathrm{b}\boldsymbol{\varepsilon}^\mathrm{bT}\rangle\mathbf{H}_2^\mathrm{T} = a\mathbf{B}\mathbf{H}_2^\mathrm{T}
$${#eq-crosscov-est}
...
for i in range(nrows):
    ...
    mp = axs[i,0].matshow(v1,cmap='coolwarm',norm=Normalize(-vlim,vlim))
    fig.colorbar(mp,ax=axs[i,0],shrink=0.6,pad=0.01)
    axs[i,0].set_title(f'a={coef_a:.2f}')
    lam, _ = la.eigh(v1)
...
nrows = len(coef_a_list)//2
ncols = 2
fig, axs = plt.subplots(figsize=[8,15],nrows=nrows,ncols=ncols,constrained_layout=True)
for i in range(nrows):
    coef_a = coef_a_list[2*i+1]
    v1 = velist[i]/(nmem-1)
