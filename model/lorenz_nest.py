...
class L05nest():
    def __init__(self, nx_true, nx_gm, nx_lam, nk_gm, nk_lam, \
        ...
        # relaxation factor
        self.po = po
        self.rlx = np.arange(self.nsp)/self.nsp # linear
...
#            plt.close()
    ...
    def __call__(self,x_gm,x_lam):
        ...
        for i in range(self.lamstep):
            ...
            t_wgt = float(self.lamstep-i-1)/float(self.lamstep)
            ...
            if self.nsp>0:
                ...
                self.bound_rlx(xl_lam_ext,xs_lam_ext,t_wgt,x0_gm2lamext,x1_gm2lamext)
            # boundary conditions
            xl_lam_ext[:self.ghost] = t_wgt*x0_gm2lamext[:self.ghost] + (1.0-t_wgt)*x1_gm2lamext[:self.ghost]
    ...
    def bound_rlx(self,xl_lam_ext,xs_lam_ext,t_wgt,x0_gm2lamext,x1_gm2lamext):
        # Davies relaxation
        ...
        if xl_lam.ndim==2:
            xl_lam[:self.nsp] = xl_lam[:self.nsp]*self.rlx[:,None] + (t_wgt*x0l_gm2lam[:self.nsp]+(1.0-t_wgt)*x1l_gm2lam[:self.nsp])*(1.0-self.rlx[:,None])
            xs_lam[:self.nsp] = xs_lam[:self.nsp]*self.rlx[:,None]
            xl_lam[-self.nsp:] = xl_lam[-self.nsp:]*self.rlx[::-1,None] + (t_wgt*x0l_gm2lam[-self.nsp:]+(1.0-t_wgt)*x1l_gm2lam[-self.nsp:])*(1.0-self.rlx[::-1,None])
            xs_lam[-self.nsp:] = xs_lam[-self.nsp:]*self.rlx[::-1,None]
        else:
            xl_lam[:self.nsp] = xl_lam[:self.nsp]*self.rlx[:] + (t_wgt*x0l_gm2lam[:self.nsp]+(1.0-t_wgt)*x1l_gm2lam[:self.nsp])*(1.0-self.rlx[:])
            xs_lam[:self.nsp] = xs_lam[:self.nsp]*self.rlx[:]
            xl_lam[-self.nsp:] = xl_lam[-self.nsp:]*self.rlx[::-1] + (t_wgt*x0l_gm2lam[-self.nsp:]+(1.0-t_wgt)*x1l_gm2lam[-self.nsp:])*(1.0-self.rlx[::-1])
...
if __name__ == "__main__":
    ...
    fig,ax=plt.subplots()
    ax.plot(step.ix_lam_ext,x0l_lam_ext,c='tab:blue')
    ax.plot(step.ix_lam_ext,x0s_lam_ext,c='tab:orange')
    x0_gm2lamext = gm2lam(step.ix_lam_ext)
    ax.plot(step.ix_lam_ext,x0_gm2lamext,c='k')
    if nsp>0:
        step.bound_rlx(x0l_lam_ext,x0s_lam_ext,0.0,x0_gm2lamext,x0_gm2lamext)
        ax.plot(step.ix_lam_ext,x0l_lam_ext,c='tab:green',ls='dashed')
        ax.plot(step.ix_lam_ext,x0s_lam_ext,c='tab:red',ls='dashed')
    ax.vlines([step.ix_lam[0],step.ix_lam[-1]],0,1,ls='dashdot',colors='k',transform=ax.get_xaxis_transform())
    ax.set_xlim(step.ix_lam_ext[-step.ghost-step.nsp*2],step.ix_lam_ext[-1])
    plt.show()#block=False)
    plt.close()
    #exit()
    ...
    fig.savefig(f"lorenz/l05nest_ng{nx_gm}nl{nx_lam}kg{nk_gm}kl{nk_lam}nsp{nsp}p{step.po}F{int(F)}c{c:.1f}.png",dpi=300)
    plt.show()
    plt.close()