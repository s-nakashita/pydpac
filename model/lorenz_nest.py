...
#            plt.close()
    ...
    def bound_rlx(self,x_lam_ext,t_wgt,x0_gm2lamext,x1_gm2lamext):
        x_lam = x_lam_ext[self.ghost:self.ghost+self.nx_lam]
        ...
        if x_lam_ext.ndim==2:
            x_lam[:self.nsp] = x_lam[:self.nsp]*self.rlx[:,None] + ((1.0-t_wgt)*x0_gm2lam[:self.nsp]+t_wgt*x1_gm2lam[:self.nsp])*(1.0-self.rlx[:,None])
            ...
            x_lam[-self.nsp:] = x_lam[-self.nsp:]*self.rlx[::-1,None] + ((1.0-t_wgt)*x0_gm2lam[-self.nsp:]+t_wgt*x1_gm2lam[-self.nsp:])*(1.0-self.rlx[::-1,None])
            ...
            x_lam[:self.nsp] = x_lam[:self.nsp]*self.rlx[:] + ((1.0-t_wgt)*x0_gm2lam[:self.nsp]+t_wgt*x1_gm2lam[:self.nsp])*(1.0-self.rlx[:])
            ...
            x_lam[-self.nsp:] = x_lam[-self.nsp:]*self.rlx[::-1] + ((1.0-t_wgt)*x0_gm2lam[-self.nsp:]+t_wgt*x1_gm2lam[-self.nsp:])*(1.0-self.rlx[::-1])
...
if __name__ == "__main__":
    ...
    for k in range(nt20d):
        ...
        if k%int(2*4*0.05/dt)==0:
            ...
            axsb[1].set_xlim(step.ix_lam_ext[-step.ghost-step.nsp*2],step.ix_lam_ext[-1])
