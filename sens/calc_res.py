...
for i,key in enumerate(resmul_dict.keys()):
    axs[0,0].plot(resmul_calc,resmul_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
    axs[0,1].plot(resuni_calc,resmul_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
    axs[1,0].plot(resmul_calc,resuni_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
    axs[1,1].plot(resuni_calc,resuni_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
for ax in axs.flatten():
    xmin, xmax = ax.get_xlim()
    xlim = max(-xmin,xmax)
    ax.set_xlim(-xlim,xlim)
    ax.set_ylim(-xlim,xlim)
    line = np.linspace(-xlim,xlim,100)
...
axs[0,0].set_xlabel('observed (multivariate)')
axs[0,0].set_ylabel('estimated (multivatiate)')
axs[0,1].set_xlabel('observed (univariate)')
axs[0,1].set_ylabel('estimated (multivariate)')
axs[1,0].set_xlabel('observed (multivariate)')
axs[1,0].set_ylabel('estimated (univariate)')
axs[1,1].set_xlabel('observed (univariate)')
axs[1,1].set_ylabel('estimated (univariate)')
fig.suptitle(r'$\Delta J(\Delta x_{0i})$'+f' vt={vt}h, Nens={nens} #{nsample}')
fig.savefig(figdir/f'resunimul_vt{vt}ne{nens}.png')
plt.show()
plt.close()
...
for i,key in enumerate(resmul_dict.keys()):
    if key=='asa': continue
    axs[0,0].plot(resmul_asa,resmul_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
    axs[0,1].plot(resuni_asa,resmul_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
    axs[1,0].plot(resmul_asa,resuni_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
    axs[1,1].plot(resuni_asa,resuni_dict[key],lw=0.0,marker=markers[key],c=colors[key],label=key,**marker_style)
for ax in axs.flatten():
    xmin, xmax = ax.get_xlim()
    xlim = max(-xmin,xmax)
    ax.set_xlim(-xlim,xlim)
    ...
    ax.set_aspect(1.0)
axs[0,1].legend(loc='upper left',bbox_to_anchor=(1.01,1.0))
axs[0,0].set_xlabel('ASA (multivariate)')
axs[0,0].set_ylabel('EnASA (multivatiate)')
axs[0,1].set_xlabel('ASA (univariate)')
axs[0,1].set_ylabel('EnASA (multivariate)')
axs[1,0].set_xlabel('ASA (multivariate)')
axs[1,0].set_ylabel('EnASA (univariate)')
axs[1,1].set_xlabel('ASA (univariate)')
axs[1,1].set_ylabel('EnASA (univariate)')
fig.suptitle(r'$\Delta J(\Delta x_{0i})$'+f' vt={vt}h, Nens={nens} #{nsample}')
...
ds.to_netcdf(savedir/f"res_calc_vt{vt}nens{nens}.nc")
