...
def confidence_ellipse(x, y, ax, n_std=3.0, facecolor='none', **kwargs):
    ...
    **kwargs
    ...
    if pearson>0:
        aspectr = ell_radius_x / ell_radius_y
        theta = np.rad2deg(np.arctan2(scale_y,scale_x))
    else:
        aspectr = ell_radius_y / ell_radius_x
        theta = np.rad2deg(np.arctan2(scale_y,-scale_x))
    ...
    return ax.add_patch(ellipse), aspectr, theta
