...
class ASA():
    ...
    def plot_hov(self):
        ...
        fig, ax = plt.subplots()
        mp = ax.colormesh(x,t[::-1],dJdx,shading='auto',norm=Normalize(-vlim,vlim),cmap='coolwarm')
        fig.colorbar(mp,ax=ax,shrink=0.6,pad=0.01)
        plt.show()

def calc_dxopt(xb,dJdx0,vt,cost,*args):
    J = cost(xb[vt],*args)
    optscale = -1.0 * J / np.dot(dJdx0,dJdx0)
    return optscale * dJdx0

def check_djdx(xb,dJdx0,dxopt,vt,model,tlm,cost,*args,title='ASA',plot=True,scale=1.0):
    xp = xb[0] + dxopt
    dxp = dxopt.copy()
    for i in range(vt):
        # nonlinear evolution
        ...
        dxp = tlm(xb[i],dxp)
    J = cost(xb[vt],*args)
    res_nl = (cost(xp,*args) - J)/J 
    res_tl = (cost(xb[vt]+dxp,*args) - J)/J
