...
#_,f_trunc,ytrunc = trunc1d(xtrunc,return_coef=True)
...
#    ax.set_ylim(0,2*r)
ax0.set_ylim(0,2*r)
fig.savefig(f"test_dst_gm_ntrunc{trunc1d.ntrunc}.new.png",dpi=300)
plt.show()
plt.close()
#exit()
fig = plt.figure(figsize=[8,4],constrained_layout=True)
...
trunc1d = Trunc1d(ix_lam,ftrunc=ftrunc,cyclic=False,ttype='s',nghost=0)#,nglobal=nx_true)
...
#    ax.set_ylim(0,2*r)
ax0.set_ylim(0,2*r)
fig.savefig(f"test_dst_lam_ntrunc{trunc1d.ntrunc}.new.png",dpi=300)
plt.show()