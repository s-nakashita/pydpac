...
xtdecomp = scale_decomp(xt,ix=ix_t_rad, kthres=kthres, axis=1)
...
for pt in perts:
    ...
    axs[0].set_title("full")
    p0 = fig.colorbar(mp0, ax=axs[0], orientation='vertical',shrink=0.6,pad=0.01)
    xgdecomp = scale_decomp(xagm,ix=ix_gm_rad, kthres=kthres, axis=1)
    ...
    axs[0].set_title("full")
    p0 = fig.colorbar(mp0, ax=axs[0], orientation='vertical',shrink=0.6,pad=0.01)
    xldecomp = scale_decomp(xalam,ix=ix_lam_rad, kthres=kthres,\
        axis=1,cyclic=False)
