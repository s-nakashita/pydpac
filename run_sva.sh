...
perturbations="letkf"
na=150 # Number of assimilation cycle
...
for op in ${operators}; do
  ...
  #    lplot=F
  #  fi
    echo ${op} ${pt} ${na} ${linf} ${lloc} ${ltlm} ${v_time} ${ntype} ${nmem}
    #python ../sva.py ${op} ${pt} ${na} ${linf} ${lloc} ${ltlm} ${v_time} ${ntype} > l96_${op}_${pt}.log 2>&1
    python ../ensvsa.py ${op} ${pt} ${na} ${linf} ${lloc} ${ltlm} ${v_time} ${ntype} ${nmem} > l96_${op}_${pt}.log 2>&1
    wait
    ...
    #rm truth.npy
  #done
  python ../plot/calc_mean.py ${op} sv${hh}h 55 50 es ${pt}
  python ../plot/calc_mean.py ${op} sv${hh}h 55 50 est ${pt}
  python ../plot/calc_mean.py ${op} random 55 50 er ${pt}
  python ../plot/calc_mean.py ${op} random 55 50 ert ${pt}
  #python ../plot/calc_mean.py ${hh} l96 10 ${count} sv ${pt}
  python ../plot/plotsv_sp.py ${pt} ${v_time} 50
  mv sv${hh}h_es_${op}_${pt}.txt sv${hh}_${pt}.txt
  mv sv${hh}h_est_${op}_${pt}.txt sv${hh}_${pt}_tlm.txt
  mv random_er_${op}_${pt}.txt random_${pt}.txt 
  mv random_ert_${op}_${pt}.txt random_${pt}_tlm.txt 
  rm er*.txt 
  rm es*.txt
  rm isv*.npy
  rm fsv*.npy
  done
  python ../plot/plotsv.py ${pt} ${ntype}
  done
