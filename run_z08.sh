...
perturbations="lb-fh lb-jh newton-fh newton-jh"
...
#  pt=${pert}
  ...
  #fi
  case $pert in
    cg-fh ) pt=mlef ; ltlm="F" ;;
    cg-jh ) pt=mlef ; ltlm="T" ;;
    lb-fh ) pt=mlef ; ltlm="F" ;;
    lb-jh ) pt=mlef ; ltlm="T" ;;
    newton-fh ) pt=mlefw ; ltlm="F" ;;
    newton-jh ) pt=mlefw ; ltlm="T" ;;
  esac
  for op in ${operators}; do
    echo ${op} ${pt} ${na} ${linf} ${lloc} ${ltlm}
    python ../../z08.py ${op} ${pt} ${na} ${linf} ${lloc} ${ltlm} > z08_${op}_${pert}.log 2>&1
    wait
    #cp z08_e_${op}_${pt}.txt z08_e_${op}_${pert}.txt
    ...
    for i in $(seq 0 $((${na} - 1))); do
      head -n 1 z08_jh_${op}_${pt}_cycle${i}.txt >> ijh_${op}_${pert}_${itest}.txt
      tail -n 1 z08_jh_${op}_${pt}_cycle${i}.txt >> fjh_${op}_${pert}_${itest}.txt
      rm z08_jh_${op}_${pt}_cycle${i}.txt
