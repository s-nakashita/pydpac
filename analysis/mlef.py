...
class Mlef():
    ...
    def analysis(self, xb, y, gtol=1e-6, 
        ...
        if save_dh:
            np.save("{}_dh_{}_{}_cycle{}.npy".format(model, self.op, self.pt, icycle), dh)
            ob = y - self.obs.h_operator(xc)
            np.save("{}_d_{}_{}_cycle{}.npy".format(model, self.op, self.pt, icycle), ob)
...
#    print("save_hist={} cycle{}".format(save_hist, icycle))
        ...
        #        jac=None, options={'gtol':gtol, 'disp':disp}, callback=callback)
            ...
            if model=="z08":
                ...
                if xmax < 1000:
                    self.cost_j(1000, xf.shape[1], model, res.x, icycle, *args_j)
                else:
                    xmax = int(xmax*0.01+1)*100
                    logger.debug("resx max={}".format(xmax))
                    self.cost_j(xmax, xf.shape[1], model, res.x, icycle, *args_j)
            elif model=="l96":
                self.cost_j(200, xf.shape[1], model, res.x, icycle, *args_j)
        else:
            res = spo.minimize(self.calc_j, x0, args=args_j, method='BFGS', \
...
#            res.fun, np.sqrt(res.jac.transpose() @ res.jac), res.nit))
        xa = xc + gmat @ res.x
        if save_dh:
            np.save("{}_dx_{}_{}_cycle{}.npy".format(model, self.op, self.pt, icycle), gmat@res.x)
        if self.pt == "grad":
        ...
        pa = pf @ tmat 
        if save_dh:
            np.save("{}_pa_{}_{}_cycle{}.npy".format(model, self.op, self.pt, icycle), pa)
            ua = np.zeros((xa.size, nmem+1))
            ua[:, 0] = xa
            ua[:, 1:] = xa[:, None] + pa
            np.save("{}_ua_{}_{}_cycle{}.npy".format(model, self.op, self.pt, icycle), ua)
        #print("xa={}".format(xa))
        if infl:
    ...
    def cost_j(self, nx, nmem, model, xopt, icycle, *args):
        xc, pf, y, tmat, gmat, heinv, rinv= args
    ...
    #np.save("{}_cJo_{}_{}_cycle{}.npy".format(model, op, pt, icycle), jvalo)
        for k in range(nmem):
            ...
            for i in range(len(delta)):
                ...
                jvalb[i+1,k] = j
        np.save("{}_cJ_{}_{}_cycle{}.npy".format(model, self.op, self.pt, icycle), jvalb)
