...
```{python}
datadir = Path(f'work/l05nestm/envar_nestc_shrink_preGM_m80obs30/data')
...
```{python}
    ...
    for mu in mulist:
        #mu = 0.01
        Pcinv = la.inv(Pc+mu*np.eye(Pc.shape[0]))
        Xb_ = Pcinv @ np.vstack((Xb,Zb))
        dv_ = Pcinv @ np.hstack((np.zeros(Xb.shape[0]),dv))
