...
class L05m_func():
    ...
    def __init__(self, model, step, obs, params):
        ...
        if len(model_params) < 5:
            self.nx, self.nks, self.dt, self.F = model_params
            logger.info("nx={} F={} dt={:7.3e}".format(self.nx, self.F, self.dt))
            logger.info("nk={}".format(self.nks))
        else:
            self.nx, self.nks, self.ni, self.b, self.c, self.dt, self.F = model_params
            logger.info("nx={} ni={}".format(self.nx, self.ni))
            logger.info("nk={}".format(self.nks))
