...
sig_l = {"mlef":8.0,"etkf":6.0,"po":2.0,"srf":8.0,"letkf":7.5,"kf":None,"var":None,
    ...
    for i in a_time:
        ...
        if i >= 100:
            ...
            # Euclidean inner product
                q, r = la.qr(np.random.randn(nx, nx))
            alpha = 1e-5
            if ft=="ensemble":
                if pt == "mlef" or pt == "4dmlef":
                    if ntype == "inner":
                    # Euclidean inner product
                        ub[:, 1:] = u0[:, None] + q[:, 0:spa.shape[1]] * alpha
                    gmat = ub[:, 1:] - u0[:, None]
                else:
                    if ntype == "inner":
                    # Euclidean inner product
                        ub = u0[:, None] + q[:, 0:spa.shape[1]] * alpha * np.sqrt(nmem-1)
                    gmat = (ub - u0[:, None]) / np.sqrt(nmem-1)
            else:
                exit()
            if count == 1:
                fig, ax = plt.subplots()
            ...
            for j in range(v_time-1):
                ...
                if ft=="ensemble":
                    if pt == "mlef" or pt == "4dmlef":
                        ...
                        ua.append(np.mean(ub, axis=1))
                else:
                    ua.append(ub)
            if ft=="ensemble":
                if pt == "mlef" or pt == "4dmlef":
                    fmat = ub[:, 1:] - ub[:, 0].reshape(-1,1)
                else:
                    fmat = (ub - np.mean(ub, axis=1).reshape(-1,1)) / np.sqrt(nmem-1)
            else:
                exit()

            p = SV.ensv(fmat, vnorm=vnorm)
            if ntype == "inner":
                v0 = gmat @ p / alpha
            else:
                v0 = gmat @ p
