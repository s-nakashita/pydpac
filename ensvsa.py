...
if __name__ == "__main__":
    ...
    if ft=="ensemble":
        if pt == "mlef" or pt == "4dmlef":
            u0 = u[:, 0]
            # Euclidean inner product
            u[:, 1:] = u0[:, None] + q[:, 0:spa.shape[1]] * alpha
            gmat = u[:, 1:] - u0[:, None]
        else:
            u0 = np.mean(u, axis=1)
            # Euclidean inner product
            u = u0[:, None] + q[:, 0:spa.shape[1]] * np.sqrt(nmem-1)
            gmat = (u - u0[:, None]) / np.sqrt(nmem-1)
    else:
        exit()
    if lplot:
        fig, ax = plt.subplots()
    ...
    if ft=="ensemble":
        if pt == "mlef" or pt == "4dmlef":
            fmat = u[:, 1:] - u[:, 0].reshape(-1,1)
        else:
            fmat = (u - np.mean(u, axis=1).reshape(-1,1)) / np.sqrt(nmem-1)
    else:
        exit()
